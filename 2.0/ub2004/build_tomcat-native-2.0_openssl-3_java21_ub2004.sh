...
_install_java21() {
    ...
    _tmp_dir="$(mktemp -d)"
    cd "${_tmp_dir}"
    _java21_ver="$(wget -qO- 'https://github.com/icebluey/javabin/releases' | grep -i '/tree/v21\.' | sed 's|"|\n|g' | grep '^/.*/tree/v' | sed 's|.*/v||g' | sort -V | uniq | tail -n 1)"
    wget -q -c -t 9 -T 9 "https://github.com/icebluey/javabin/releases/download/v${_java21_ver}/jdk-${_java21_ver}_linux-x64_bin.tar.gz.sha256"
    wget -q -c -t 9 -T 9 "https://github.com/icebluey/javabin/releases/download/v${_java21_ver}/jdk-${_java21_ver}_linux-x64_bin.tar.gz"
    sha256sum -c "jdk-${_java21_ver}_linux-x64_bin.tar.gz.sha256"
    rm -fr /usr/java/jdk
    [[ -d /usr/java/jdk-"${_java21_ver}" ]] && rm -fr /usr/java/jdk-"${_java21_ver}"
    install -m 0755 -d /usr/java
    tar -xof "jdk-${_java21_ver}_linux-x64_bin.tar.gz" -C /usr/java/
    ln -sv jdk-"${_java21_ver}" /usr/java/jdk
...
sleep 2
cd /tmp
tar -zcvf /tmp/"tomcat-native-${_tcn20_ver}_openssl-${_openssl35_ver}_java21-1.ub2004.amd64.tar.gz" tomcat-native
echo
sleep 2
cd /tmp
openssl dgst -r -sha256 tomcat-native-${_tcn20_ver}_openssl-${_openssl35_ver}_java21-1.ub2004.amd64.tar.gz | sed 's|\*| |g' > tomcat-native-${_tcn20_ver}_openssl-${_openssl35_ver}_java21-1.ub2004.amd64.tar.gz.sha256
rm -fr "${_tmp_dir}"
rm -fr /tmp/tcn20
rm -fr /tmp/tomcat-native
